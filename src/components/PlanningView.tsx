...
function SavingsPanel({ data, ownerId, putGoal, putAllocation, archiveGoal, reference }: PlanningPanelProps) {
  ...
  const allocatedToGoal = (id: string) => sumMoney(data.allocations
  ...
  const createGoal = (event: FormEvent) => {
    ...
    if (!name.trim() || amount === null || amount <= 0) return setMessage('請輸入目標名稱與大於 0、最多兩位小數且可安全精確處理的金額');
    const applied = putGoal({ ...newRecordMeta(ownerId), name: name.trim(), targetAmount: amount, targetDate: targetDate || undefined, isActive: true });
  ...
  const allocate = (event: FormEvent) => {
    ...
    if (!resolvedGoalId || amount === null || amount <= 0) return setMessage('請選擇目標並輸入大於 0、最多兩位小數且可安全精確處理的金額');
    if (compareMoney(amount, financials.availableAssets) > 0) return setMessage(`可配置資產僅有 ${money.format(financials.availableAssets)}，未建立配置`);
    const applied = putAllocation({ ...newRecordMeta(ownerId), goalId: resolvedGoalId, amountDelta: amount, occurredAt: toLocalInput(), note: '手動配置' });
    completeAppliedMutation(applied, () => {
  ...
  return (
    <>
      <section className="hero-card hero-card-savings" aria-labelledby="savings-overview-title"><div className="relative z-10"><p id="savings-overview-title" className="text-sm font-bold text-white/75">資產配置</p><p className="mt-1 text-3xl font-black">已配置 {money.format(financials.allocatedSavings)}</p><p className="mt-2 text-sm text-white/80">總資產 {money.format(financials.totalAssets)} · 可配置 {money.format(financials.availableAssets)}</p>{compareMoney(financials.availableAssets, 0) < 0 && <p className="mt-3 flex items-center gap-2 rounded-xl bg-rose-950/30 p-2 text-sm"><AlertTriangle className="h-4 w-4" />配置可能因離線同步或資產變動而高於目前資產；請先釋放部分配置，再新增配置。</p>}</div></section>
...
function BudgetPanel({ data, ownerId, putBudget, archiveBudget, reference }: PlanningPanelProps) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (limit === null || limit <= 0 || (scope === 'category' && !category)) return setMessage('請輸入大於 0、最多兩位小數且可安全精確處理的預算並選擇分類');
    ...
    completeAppliedMutation(applied, () => {
      setAmount(''); setMessage(existing ? '已更新相同範圍預算' : '預算已建立');
    }, setMessage);
  };
