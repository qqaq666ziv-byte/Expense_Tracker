...
export function SettingsView(props: SettingsViewProps) {
  ...
  const items: { key: Section; label: string }[] = [{ key: 'accounts', label: '帳戶' }, { key: 'categories', label: '分類' }, { key: 'recurring', label: '週期' }, { key: 'backup', label: '備份' }];
  return <div className="space-y-5"><div className="grid grid-cols-4 rounded-2xl bg-white p-1 shadow-sm dark:bg-zinc-900">{items.map((item) => <button type="button" className={`tab-button ${section === item.key ? 'tab-button-active' : ''}`} key={item.key} aria-pressed={section === item.key} onClick={() => setSection(item.key)}>{item.label}</button>)}</div>{section === 'accounts' && <AccountsPanel {...props} />}{section === 'categories' && <CategoriesPanel {...props} />}{section === 'recurring' && <RecurringPanel {...props} />}{section === 'backup' && <BackupPanel {...props} />}</div>;
}

function AccountsPanel({ data, ownerId, putAccount, archiveAccount }: SettingsViewProps) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (editing
      ...
      && !window.confirm(`修改「${editing.name}」期初餘額會重新計算所有歷史衍生餘額。若只是對帳差異，應改用首頁「餘額校正」。確定繼續？`)) {
      setMessage('未修改期初餘額');
      return;
    }
  ...
  return <div className="grid gap-5 lg:grid-cols-2"><section className="card"><div className="section-heading"><div><p className="eyebrow">穩定 ID · 帳本衍生餘額</p><h2>{editing ? '編輯資產帳戶' : '新增資產帳戶'}</h2></div></div><form className="space-y-3" onSubmit={submit}><label className="field-label">名稱<input aria-label="帳戶名稱" className="field mt-1" value={name} onChange={(event) => setName(event.target.value)} /></label><label className="field-label">期初餘額<input aria-label="期初餘額" className="field mt-1" inputMode="decimal" value={opening} onChange={(event) => setOpening(event.target.value.replace(/[^0-9.-]/g, ''))} /></label><label className="flex items-start gap-3 rounded-xl bg-amber-50 p-3 text-sm dark:bg-zinc-800"><input aria-label="納入總資產" className="mt-1 h-4 w-4 accent-amber-600" type="checkbox" checked={included} onChange={(event) => setIncluded(event.target.checked)} /><span><strong className="block">納入總資產</strong><span className="text-xs text-zinc-500">只勾選現金、電子錢包等真正持有資產；信用卡／債務不在本版本建模範圍。</span></span></label>{editing && data.transactions.some((item) => !item.deletedAt && item.accountId === editing.id) && <p className="warning-message">此帳戶已有交易；修改期初餘額會改變所有衍生餘額。若只是實際餘額不同，請使用首頁「餘額校正」。</p>}<IconPicker value={icon} onChange={setIcon} /><div className="flex gap-2"><button className="primary-button flex-1" type="submit"><Save className="h-4 w-4" />儲存帳戶</button>{editing && <button type="button" className="secondary-button" onClick={() => { setEditing(null); setName(''); setIncluded(true); }}>取消</button>}</div>{message && <p aria-live="polite" className="text-sm text-zinc-500">{message}</p>}</form></section><section className="card"><div className="section-heading"><div><p className="eyebrow">封存不刪歷史</p><h2>帳戶清單</h2></div></div><div className="space-y-2">{accounts.map((account) => <article className={`settings-row ${account.isActive ? '' : 'opacity-55'}`} key={account.id}><span className="icon-badge"><FinanceIcon icon={account.icon} /></span><button type="button" className="min-w-0 flex-1 text-left" onClick={() => edit(account)}><strong className="block truncate">{account.name}</strong><span className="text-xs text-zinc-500">期初 {money.format(account.openingBalance)}{!account.includeInTotalAssets ? ' · 未納入總資產' : ''}{account.requiresReview ? ' · 待確認' : ''}</span></button>{account.isActive ? <button className="icon-button" type="button" disabled={activeCount <= 1} aria-label={`封存 ${account.name}`} title={activeCount <= 1 ? '至少保留一個可用帳戶' : '封存'} onClick={() => archiveAccount(account)}><Archive className="h-4 w-4" /></button> : <button type="button" className="secondary-button" onClick={() => putAccount({ ...account, ...changedRecordMeta(account), isActive: true })}>啟用</button>}</article>)}</div></section></div>;
}

function CategoriesPanel({ data, ownerId, putCategory, archiveCategory }: SettingsViewProps) {
