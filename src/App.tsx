...
import { BarChart3, Cloud, CloudOff, Home, LogIn, LogOut, RefreshCw, Settings, Target } from 'lucide-react';
...
export default function App() {
  ...
  const archiveAccount = (record: FinanceData['accounts'][number]) => {
    ...
    if (!window.confirm(`確定封存「${record.name}」？${impact}。`)) return;
    app.put('accounts', { ...record, ...changedRecordMeta(record), isActive: false });
    data.recurringRules
      .filter((rule) => !rule.deletedAt && rule.isActive && rule.accountId === record.id)
      .forEach((rule) => app.put('recurringRules', { ...rule, ...changedRecordMeta(rule), isActive: false }));
  };
  const archiveCategory = (record: FinanceData['categories'][number]) => {
    ...
    if (recurringCount > 0 && !window.confirm(`封存「${record.name}」會同時暫停 ${recurringCount} 條週期規則；歷史交易仍會保留。是否繼續？`)) return;
    app.put('categories', { ...record, ...changedRecordMeta(record), isActive: false });
    data.recurringRules
  ...
  return (
    <div className="min-h-screen pb-24 text-zinc-900 dark:text-zinc-100">
      <header className="sticky top-0 z-30 border-b border-amber-100/80 bg-[#fffaf0]/90 backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/90">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button type="button" className="status-pill" disabled={app.state.ownerId === 'guest' || app.syncBusy} onClick={() => void app.syncNow()} title={app.state.lastSyncError ?? '同步狀態'} aria-label={app.state.ownerId === 'guest' ? '僅此裝置' : !online ? `離線，待傳 ${pending} 筆` : pending > 0 ? `待同步 ${pending} 筆` : '已同步'}>
              ...
              <span className="hidden sm:inline">{app.state.ownerId === 'guest' ? '僅此裝置' : !online ? `離線 · 待傳 ${pending}` : pending > 0 ? `待同步 ${pending}` : '已同步'}</span>
            </button>
      ...
      <main className="mx-auto max-w-5xl px-4 py-5">
        ...
        {app.storageRecovery ? <div className="warning-banner"><div><strong>本機快照已進入復原保護</strong><p>{app.storageError} 請先下載原始快照，再從「管理 → 備份」還原有效備份；成功還原前不會覆寫原始 key。</p></div><button type="button" className="secondary-button" onClick={() => {
        ...
        <Suspense fallback={<div className="card text-center text-sm text-zinc-500">載入功能中…</div>}>
