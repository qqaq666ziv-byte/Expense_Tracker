...
- **真的快**：金額是主角；手機預設叫出 0–9 數字鍵盤並即時顯示千分位，小數與負數只在需要時開啟。常用分類與資產帳戶直接點選，時間預設現在，備註收在次要層級。
...
- **真的可復原**：完整 JSON 備份、安全合併或明確取代還原；收支與轉帳各有獨立 CSV 可供自行整理。

## 產品體驗

### 記帳

支出／收入／轉帳三種第一級紀錄共用主角式金額輸入。收支使用分類與資產帳戶；轉帳則明確選擇來源與目的帳戶，可一鍵交換方向，而且不會被算成收入或支出。重複出現的真實帳本紀錄會在本機形成常用備註與「再記一次」建議；使用者也能建立只保存在目前瀏覽器、依帳本 owner 隔離的備註快捷。這些便利功能只填入表單，仍須明確儲存，也不會成為財務來源資料或同步到雲端。輸入 `3307` 會自然顯示 `NT$ 3,307`；所有 money input 共用安全字串清理與既有精度驗證，不使用會接受 `e`、`+`、`-` 的 `type=number`。最近紀錄支援搜尋、編輯、刪除與漸進載入完整歷史。銀行或 ATM 手續費是獨立支出，不會折入轉帳金額。
...
「柴柴陪你完成第一筆記帳」會直接帶使用者操作正式介面的建立、定位、編輯與 tombstone 刪除流程。教學紀錄有保留標記，會從正常帳本、資產、預算、分析、CSV 與訪客匯入排除；完整 JSON 備份仍保留必要的內部同步證據。教學可跳過、暫停、續跑、完整重跑，也能從「幫助與教學」單獨重跑第一筆、今日快照、資產、洞察、規劃或同步章節。複雜頁面另有只出現一次的情境提示與簡短 FAQ。

## 資料與同步

```text
帳戶餘額   = 起始金額 + 收入 - 支出 + 轉入 - 轉出 + 餘額調整
總資產     = 所有啟用且計入總資產的帳戶餘額
可配置資產 = 總資產 - 已配置儲蓄
```
...
- 離線建立轉帳或改選端點後，若在上雲前偵測到所選帳戶已被其他裝置更新，會保留原始選擇並停止上傳；使用者必須重新開啟並明確確認兩端帳戶，才會刷新名稱快照與同步版本。
...
目前不支援週期轉帳、轉帳手續費自動化、多幣別、信用卡債務、銀行串接、OCR、CSV 匯入或家庭共享。週期收支在開啟 App、回到前景或跨日後補齊，不是背景排程服務。`finance_resource_abuse_guards` 與 `finance_account_transfers` 兩支 migration 仍需另行授權，尚未套用 Production；詳細狀態請見 [supabase/ROLLBACK.md](supabase/ROLLBACK.md)。
