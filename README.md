...
支出／收入／轉帳三種第一級紀錄共用主角式金額輸入。收支使用分類與資產帳戶；轉帳則明確選擇來源與目的帳戶，可一鍵交換方向，而且不會被算成收入或支出。輸入 `3307` 會自然顯示 `NT$ 3,307`；所有 money input 共用安全字串清理與既有精度驗證，不使用會接受 `e`、`+`、`-` 的 `type=number`。最近紀錄支援搜尋、編輯、刪除與漸進載入完整歷史。銀行或 ATM 手續費是獨立支出，不會折入轉帳金額。
