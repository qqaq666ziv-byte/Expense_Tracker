-- Optional transfer fees are debited atomically from the source account.
-- Existing transfers retain zero fees; no historical amount is rewritten.
begin;
set local lock_timeout = '10s';
set local statement_timeout = '5min';
alter table public.transfers add column if not exists fee numeric not null default 0;
alter table public.transfers drop constraint if exists finance_v3_transfers_fee_numeric_chk;
alter table public.transfers add constraint finance_v3_transfers_fee_numeric_chk check (
  fee >= 0 and fee <= 100000000
  and pg_catalog.pg_column_size(fee) <= 32
  and fee = pg_catalog.round(fee, 6)
);

-- Old clients omit fee from UPSERTs. A missing value must preserve any existing
-- fee rather than silently resetting it. New clients send an explicit zero to
-- remove a fee. Resolve this before NOT NULL and conflict-clock checks.
alter table public.transfers alter column fee drop default;
create or replace function finance_private.preserve_omitted_transfer_fee()
returns trigger
language plpgsql
security invoker
...
begin
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('finance-transfer-fee:' || new.user_id::text || ':' || new.id, 0)
  );
  if new.fee is null then
    if tg_op = 'UPDATE' then
      new.fee := old.fee;
    else
      select fee into new.fee from public.transfers
      where user_id = new.user_id and id = new.id
      for update;
      new.fee := coalesce(new.fee, 0);
    end if;
  end if;
  return new;
end
$function$;
revoke all on function finance_private.preserve_omitted_transfer_fee()
  from public, anon, authenticated;
drop trigger if exists finance_v3_05_transfer_fee on public.transfers;
create trigger finance_v3_05_transfer_fee before insert or update on public.transfers
for each row execute function finance_private.preserve_omitted_transfer_fee();
  ...
  total_assets_minor_units := total_assets_minor_units + coalesce((
    select sum(
      case when exists (
        ...
        - pg_catalog.round(transfer.fee * 100, 0) else 0 end
      + case when exists (
